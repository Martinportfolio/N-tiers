...
class IRCServer {
    constructor(port = 6667) {
        ...
        this.server = null; // Ajout d'une référence au serveur
    }
    ...
    start() {
        ...
        this.server = server; // Stocker la référence du serveur
    ...
    // Nouvelle méthode pour gérer les signaux
    setupSignalHandlers() {
        const signals = ['SIGINT', 'SIGTERM'];
        
        signals.forEach(signal => {
            process.on(signal, () => {
                console.log(`\nSignal ${signal} reçu. Arrêt du serveur dans 5 secondes...`);
                
                // Informer tous les clients
                for (const [clientSocket, _] of this.clients) {
                    this.sendToClient(clientSocket, "Le serveur va s'arrêter dans 5 secondes.\n");
                }
